...
:Purpose:   This program is used to download Python packages from PyPI
            ...
            they - along with the testing report/log - are added to a
            .zip file and stored on the user's Desktop for transfer to
...
from ppk.libs.tests import Tests
...
class PPKPacker:
    ...
    def main(self):
        ...
        :Processes:
            ...
            - Build the name of the output files, .zip and the log.
            ...
            - If the tests pass, bundle the package and its dependencies
              in a .zip file on the user's desktop.
    ...
    def _create_archive(self):
        """Create an archive ``.zip`` file from the contents of the temp
        directory.

        If the verification has passed, the new zip file is created on
        the user's Desktop. Otherwise, a zip archive is not created.

        """
        if self._pass:
            files = glob(os.path.join(self._tmpdir, '*'))
            path = os.path.join(utilities.get_desktop(), f'{self._ofname}.zip')
    ...
    def _get_package_version_number(self):
        ...
        This version is used for naming the .zip and other associated
        files.
    ...
    def _make_download_directory(self):
        ...
        self._tmpdir = os.path.join(os.path.realpath('.'), os.urandom(8).hex())
    ...
    def _print_summary(self):
        ...
        print('\nProcessing complete.',
              ...
              sep='\n')
        if self._pass:
            print('There is a .zip file on your desktop which contains the checked packages',
                  'along with the integrity check log file. This .zip file can be transferred',
                  'to the destination and unpacked.',
                  '',
                  sep='\n')
        else:
