...
class ArgParser:
    ...
    _H_PKGN = (
               ...
               'If *unpacking*:\n'
               '  - The full path to the .zip file to be unpacked.\n\n'
               ...
               'If the path to a .zip file is received, the unpacker is called\n'
               'and all other arguments are ignored. If the unpack is successful\n'
               'the cluster\'s pip repository is automatically refreshed.'
              )
    ...
    def _test_package(self):
        ...
        :Actions:
            ...
            - If the argument provided to the ``package`` parameter has a
              ``.zip`` file extension (indicating an archive to be passed
              to the unpacker) *and* the file exists, the new 'unpack'
              argument is set to True; otherwise it remains as False.

        Raises:
            FileNotFoundError: If either the TXT or ZIP files do not
            exist.
        ...
        elif os.path.splitext(arg)[1].lower() == '.zip':
            if not os.path.exists(arg):
