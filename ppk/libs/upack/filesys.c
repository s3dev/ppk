...
/**
    ...
    @return             - The number of files moved successfully.
                        - Or -1 if the number of encountered files is not 
                          equal to the number of files moved.
                        - Or -2 if the directory opening fails.
*/
...
/**
    Remove all files under the given path.
...
/**
    ...
    @param[in]  fpath   Explicit file path to the .zip file to be tested 
                        and unpacked.
...
int unzip(const char *fpath, const char *opath) {
    ...
    sprintf(cmd_tst, "unzip -tqq %s", fpath);
    sprintf(cmd_ext, "unzip -qo %s -d %s", fpath, opath);
    ...
    if ( (system(cmd_tst) == 0) ) {
        ...
        if ( (system(cmd_ext)) == 0 ) {
            ...
            reporterror("An error occurred while unpacking the .zip file.", false, true);
        }
    } else {
