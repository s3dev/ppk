/**
    ...
    Purpose:    The upack program provides the Python library bundle 
                ...
                integrity of the Python library bundle (.zip file) which
                was created by ppk's 'pack' program. Once the integrity of
                the .zip file is verified, the contents of the archive are
                transferred into the local pip repository. If the contents
                of the archive fails verification, the user is alerted and
...
int verify_args(int argc, const char *argv[]) {
    ...
    if ( (fp = fopen(argv[1], "r")) == NULL ){
        reporterror("The provided file cannot be found.", false, true);
    }
    fclose(fp);
    
    // Verify the file has a .zip extension.
    if ( ((ext = strrchr(argv[1], '.')) == NULL) || (strcmp(ext, ".zip")) ) {
        reporterror("A .zip file is required, please refer to the program usage.", true, true);
    }
    return EXIT_SUCCESS;
}
